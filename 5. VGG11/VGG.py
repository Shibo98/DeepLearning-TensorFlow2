...
is_gpu = int(input('Use GPU or not(0/1) :'))
version = int(input('Choose the version(11/13/16/19):'))
...
db = db.shuffle(1000).map(preprocess).batch(10).repeat(10)
...
if is_gpu == 1:
    with tf.device('/gpu:0'):
        # 2.网络搭建
        # network = get_VGG11() # Use VGG11
        # network = get_VGG13()   # Use VGG13

        # network = load_model('Pokemon_VGG11.h5')  # Use VGG11.h5
        network = load_model('Pokemon_VGG13.h5')  # Use VGG13.h5
        '''
        if version == 11:
            network = get_VGG11()   # Use VGG11
        elif version == 13:
...
# 4.可视化
# create dataframe
data = {'step': x_step, 'acc': y_accuracy, 'loss': y_loss}
df = pd.DataFrame(data)

ax1 = sns.lineplot(x='step', y='acc', data=df)
plt.show()
ax2 = sns.lineplot(x='step', y='loss', data=df)
...
# 5.模型保存
if version == 11:
    network.save('Pokemon_VGG11_1.h5')
elif version == 13:
    network.save('Pokemon_VGG13.h5')
elif version == 16:
    network.save('Pokemon_VGG16.h5')
